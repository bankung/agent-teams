...
| Mobile remote access (Tailscale) + push notifications | Production | Tailscale VPN still works for board/API access. ntfy removed #2756; push is now Telegram (#2757) + web push (VAPID). Helper scripts `bin/tailscale-status.*` + `bin/remote-url.*` removed #2756. | USAGE-POWER #4 |
