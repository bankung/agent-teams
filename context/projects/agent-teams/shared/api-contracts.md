...
**`SubagentModelEntry`** (Kanban #887, 2026-05-13) — `{agent:str (min_length=1), model:"opus"|"sonnet"|"haiku"|"fable", at:datetime (ISO-8601 UTC)}`. `fable` (#3341) is a log-only verify/validate tier — NOT a `model_override` value. JSONB element shape; `extra="forbid"` rejects unknown keys at 422. Lead populates `at` with UTC spawn timestamp; `agent` is the agent name from frontmatter. Full-replace PATCH semantics — Lead accumulates and sends the whole list.
