...
- 2026-09-25 — #3341 `tasks.subagent_models[].model` accepts `fable` (verify/validate-only tier, operator ruling 2026-09-25) — ONLY the spawn log widens; `model_override` / agent-frontmatter / agent-override tiers and `pricing.py` stay opus/sonnet/haiku (`model_override='fable'` → 422, test-locked). Partially supersedes #2301 item 4 (Fable-5 descope). Pricing = skip (`lookup_price` → None, drops out of auditor `coverage_pct`), no guessed price. **GOTCHA** removing `fable` from the Literal later 500s every GET on a task with a stored fable entry — backfill first
- 2026-09-25 — #3336 `projects` read routes are active-only — `GET /{id}`, `GET /by-name/{name}` + POST `/grant-consent`, `/reconcile-budget` all 404 on a soft-deleted row — a DELIBERATE deviation from `standards/postgresql/soft-delete.md` made at #691 and locked by `test_get_project_by_id_404_for_soft_deleted`; docs-only fix, no behaviour change. The standard's recovery-flow concern does not bite: `PATCH /{id}` and `DELETE /{id}` do NOT filter status (verified live — `PATCH {}` on soft-deleted id=728 → 200 with the row). **GOTCHA** `by-name` is the stronger case and NOT the same decision as `by-id`: the partial unique index lets a new project reclaim a deleted name, so unfiltered by-name is ambiguous by construction. `tasks` + `task_templates` follow the standard (repo-wide grep). `shared/api-contracts-core.md` + `api-contracts.md` carried the wrong general rule and now carry the exception; the standard gained the matching **Exception** sub-bullet 2026-09-25 (operator `ii`)
- 2026-09-24 — #3327 a PreToolUse hook's `permissionDecision: "allow"` is a permission BYPASS; emitting nothing (bare `exit 0`) is "no decision" and falls through to the allowlist + ordinary prompt — **a guard wired onto a new path must reproduce that path's previous no-match semantics exactly** (the PowerShell branch nearly shipped default-allow; caught by dev-reviewer, pinned by negative-control rows). ONE PreToolUse entry on matcher `Bash|PowerShell` runs `pretooluse-bash-gate.ps1` (measured 4 processes = 2,114 ms vs 1 = 498 ms per call); only `approval-policies-gate` (GUARD 6) is skipped for non-Bash; name kept for its references. The four `block-*.ps1` files are UNWIRED reference copies (NOT WIRED banners). curl-DELETE guard now matches `Invoke-WebRequest`/`Invoke-RestMethod`/`irm`/`iwr -Method DELETE`; live-DB guard matches `$env:DATABASE_URL = ...`; all 16 bare `Bash(docker compose ...)` entries pinned to `-p agent-teams`. Commit `49001a4`
- 2026-09-24 — #3324 the `PowerShell` matcher gets the live-DB / raw-SQL-DML / curl-DELETE guards it never had (a hook deny outranks an allowlist allow); all 7 `PowerShell(docker compose ...)` entries pinned to `-p agent-teams`, and `-p agent-teams` is a scaffold `_AGENT_TEAMS_ALLOW_DROP_SUBSTRINGS` entry (a bare compose resolves its project from the CWD basename). **Correction:** the `alembic upgrade head` entry was never an unguarded live migration — `env.py` refuses non-`_test` DBs without `MIGRATION_TARGET=live`. **GOTCHA** the guards match command TEXT, not intent — route quoting text through a file; hook wiring is live immediately. Its 4-process wiring is superseded by #3327
- 2026-09-24 — #3321 a project's build/test facts live ONLY in `<working_path>/shared/README.md` under `## Project facts (compose + tests)` (4 rows: compose name · test command · test DB · app URL; `working_path` NULL → `context/projects/<name>/shared/`); universal `CLAUDE.md` degenericised — `-p agent-teams` web rebuild, hard-coded live DB `agent_teams`, bare `docker compose exec … scripts.seed` (**no `-p`** → hits the current folder's stack) and the 7-step "Add a team" list (→ `shared/adding-a-team.md`) all out; Bootstrap step 5 now names `shared/README.md` as the source. Scaffold RENDERS a per-project CLAUDE.md pointer stub (`render_project_claude_stub`, both call sites) instead of copying the repo's own — the copy was dead-on-arrival (manifest never shipped `.claude/docs/**` + `.claude/teams/**`; MorAI measured 24/26 dead links). **GOTCHA** stub must contain zero `](` (checked in tests + init smoke); on-disk path is guarded by `"CLAUDE.md" in report.copied` to keep idempotent-add. MorAI swept (18,345 B → 1,155 B); novel-drift skipped (operator); NewsAnalyzer untouched (hand-written, not a harness copy)
- 2026-09-11 — #3171 cryptography `>=48.0.1,<49` → `>=50.0.1,<51` (lock 50.0.1): PYSEC-2026-3552 fixed only in 50.0.0 (3553/3554 in 49.0.0); old ceilings were "next major", not incompatibility; `<51` = msal's own cap; pip-audit clean. Same task: `RateLimitError(scope=tag|project)` + ingest log `bucket=` (429 body unchanged); `link_probe` overall `asyncio.wait_for` 10s deadline (slow-drip)
- 2026-09-11 — #2839+#3170+#3169 v0.8.1 release gate (from #3166 review): `/invoke` L17 scan → 422 + langgraph host port 8465 dropped (**supersedes #2503 AC2 for langgraph**); langgraph prod image non-root uid 1000 (mirror #2529; `safe.directory /repo` for non-root git); aiohttp 3.14.3 lock bump; **GOTCHA** the langgraph worker has no disable switch — pin `LANGGRAPH_PROJECT_ID` + stop after checks; `up -d --build X` recreates changed `depends_on` services → `build X` + `up -d --no-deps X`
