...
- 2026-09-25 — #3336 `projects` read routes are active-only — `GET /{id}`, `GET /by-name/{name}` + POST `/grant-consent`, `/reconcile-budget` all 404 on a soft-deleted row — a DELIBERATE deviation from `standards/postgresql/soft-delete.md` made at #691 and locked by `test_get_project_by_id_404_for_soft_deleted`; docs-only fix, no behaviour change. The standard's recovery-flow concern does not bite: `PATCH /{id}` and `DELETE /{id}` do NOT filter status (verified live — `PATCH {}` on soft-deleted id=728 → 200 with the row). **GOTCHA** `by-name` is the stronger case and NOT the same decision as `by-id`: the partial unique index lets a new project reclaim a deleted name, so unfiltered by-name is ambiguous by construction. `tasks` + `task_templates` follow the standard (repo-wide grep). `shared/api-contracts-core.md` + `api-contracts.md` carried the wrong general rule and now carry the exception; `context/standards/**` is humans-only and still has no exception clause — operator call
