...
- test command: `docker compose -p agent-teams exec -T api pytest -q` — **operator-run.** An in-session invocation is denied by the L1.5 live-DB hook (`.claude/hooks/block-pytest-on-live-db.ps1`), which cannot read the container's `DATABASE_URL` from outside. The operator verifies `docker compose -p agent-teams exec api printenv DATABASE_URL` and sets `DOCKER_PYTEST_VERIFIED=1` in the same shell before running it.
