...
- **Fix the visibility, keep the protection.** Bitdefender ATD stays active. Detection-based approach (#1463) pushes a Telegram alert to operator's phone whenever a block event fires (#2757 swapped the channel from ntfy), so operator (and Lead) know to re-run the affected command.
...
2. **Install the block-event poller** — see #1463 for implementation. Polls Bitdefender event log every 5-10min, posts to Telegram when a "Malicious command line detected" event fires for the harness PS pattern (#2757 swapped ntfy -> Telegram).
3. **When a Telegram alert fires:** identify the affected harness invocation (timestamp + truncated command line in alert body), manually re-run the affected work. Until #1463 lands, operator periodically checks Bitdefender Notifications / History by hand.

### Verify (detection mechanism — pending #1463)

After #1463 lands: trigger a known-blockable pattern → expect a Telegram message on phone within one polling cycle. Until then: blocks remain silent; operator's signal is "agent claims success but no diff lands" or "context file vanishes".
...
- `#1192` / `#2757` (push channel: originally ntfy; swapped to Telegram in #2757; #1463 poller reuses this channel)
