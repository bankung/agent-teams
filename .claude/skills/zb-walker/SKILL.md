...
description: >-
  ...
  "drain the board", "auto-run", "zb-walker", "keep working tasks until you're stuck/done".
...
# /zb-walker — Mode A continuous walker

*"Powered by Caffeine, Driven by Deadlines."* A continuous work-drain loop layered over the
existing primitives (`next-autorun` picker, `zb-tasks-next` ordering, the bootstrap binding, the
...
- **Bound project:** resolve via `powershell -File bin/lead-project-id.ps1` (this session's id). If
  unbound → run `/zb-bind` first, then re-invoke.
...
continue, not restart). Across `SCOPE` + ties, order by the `zb-tasks-next` policy
...
- **If a fresh `next_task`:** open → `in_progress` (PATCH `process_status=2`), run the lifecycle —
  ...
  `decisions.md` + the activity rail (`/zb-report`).
- **Verify EVERY acceptance criterion** before the DONE flip; PATCH the AC array with verdicts, then
  `process_status=5` (use `/zb-task-done`). If any AC is unmet → it's a **stuck** condition (§3), not
  a DONE.
- **Commit locally** when the task's work is a committable unit (`/zb-git-commit` — scoped staging +
...
## Related
- `/zb-tasks-next` — the ordering policy this loop reuses.
- `/zb-task-done` · `/zb-git-commit` · `/zb-report` — the per-task lifecycle paved paths.
