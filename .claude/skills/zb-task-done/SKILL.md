...
# /zb-task-done — verify acceptance criteria, then close
...
If it exits non-zero: STOP and run `/zb-bind <project>` first.
...
## Related skills
- `zb-task-update` — make non-DONE status changes (in_progress, review, cancelled) without the AC gate
- `zb-task-create` — create the task that will eventually be closed here
