...
1. Invoke zb-email `sweep-jobs` — this searches both Gmail and Outlook for application-response mail.
   It returns threads grouped by type (response/interview/reject/no-update). Use its output directly.
2. For any thread that needs more body context, invoke `zb-email read <id>` or `zb-email thread <id>`.
