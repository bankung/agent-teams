...
1. Invoke zb-email to pull job alert emails from both inboxes:
   - Gmail: `zb-email search "from:jobsdb OR from:michaelpage subject:(job alert OR new jobs)" --cap 30`
   - Outlook: equivalent KQL search via zb-email
2. For each alert email, invoke `zb-email read <id>` to get the body (alert emails contain listing summaries).
