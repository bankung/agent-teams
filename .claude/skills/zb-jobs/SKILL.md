...
# /zb-jobs — job-search operations playbook
...
**zb-jobs owns job LOGIC. zb-email owns email MECHANICS.**

- For any verb that reads email (mine-alerts, sweep): compose zb-email via its verbs
  (`search`, `read`, `thread`, `sweep-jobs`, `open --jobs`). Never call
  `curl http://localhost:8456/api/tools/email/*` directly from this skill.
- zb-jobs ONLY processes what zb-email returns: it matches roles, deduplicates, classifies
  responses, updates the tracker, and presents findings. The email transport layer is
  zb-email's domain entirely.
- zb-email requires `X-Project-Id: 599`. Pass that binding when composing.
...
| Situation | Action |
|---|---|
| zb-email not authenticated | Run `zb-email auth-status` first; report OAuth URL; stop. |
