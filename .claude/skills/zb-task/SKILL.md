...
description: >-
  ...
  agent-teams project. Read-only. Handy to review a task (and its AC) before /zb-task-done.
...
# /zb-task — show a single task, formatted
...
## Related skills
- `zb-tasks-next` — show the prioritized queue of what to work next (vs. a single task by id)
- `zb-task-update` — mutate a task's status, priority, or fields once you've reviewed it here
- `zb-task-done` — verify AC and close a task after inspecting it with this skill
