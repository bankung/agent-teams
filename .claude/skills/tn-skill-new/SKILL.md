...
name: tn-skill-new
description: >-
  Scaffold a new standard-compliant tn-* skill from a validated template so future skills
  start compliant-by-construction instead of hand-rolled. Use when the operator says "new
  skill", "scaffold a skill", "create a tn-* skill", "add a skill", "I want to author a
  skill", "make a new slash command", or "add a tn-<verb>".
...
# /tn-skill-new — scaffold a compliant tn-* skill

`$ARGUMENTS` = `<skill-name> [category]` (e.g. `/tn-skill-new tn-deploy kanban`).
...
| Input | Validation |
|---|---|
| `name` | Must be `tn-<verb>` (kebab-case, no spaces). Reject anything else. |
...
- **tn-spec** — spec the work before creating a task; run before this skill if the new skill's purpose is fuzzy.
...
```
/tn-skill-new tn-deploy platform
/tn-skill-new tn-notify kanban
/tn-skill-new tn-archive secretary
```

## Related skills

- **tn-spec** — harden a fuzzy task idea before creating a task; run before this skill if the new skill's scope is unclear.
- **tn-task-create** — open the Kanban task for the new skill's implementation after scaffolding.
