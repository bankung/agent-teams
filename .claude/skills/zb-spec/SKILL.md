...
description: >-
  ...
  then hand the refined spec to /zb-task-create. Use when an idea is fuzzy or high-stakes and you
...
# /zb-spec — pushback + revise twice, then create
...
**/zb-task-create** (or POST directly) so the footgun guards apply (project_id in body, AC at creation).
...
## Usage
```
/zb-spec add rate-limiting to the email endpoints
```

## Related skills

- **zb-task-create** — the skill zb-spec hands off to after hardening; creates the task with the refined spec and AC in one POST.
- **zb-intense-review** — the post-implementation counterpart to zb-spec; where zb-spec hardens the plan, zb-intense-review hardens the finished change.
