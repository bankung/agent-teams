...
# /zb-task-create — paved-path Kanban task creation
...
### Boundary vs `/zb-spec` (so the two never overlap)

`/zb-spec` = 2-round **adversarial pre-idea hardening** of a *fuzzy* idea, before you even know the
AC. This Step 2.5 = a **light review of an already-drafted task** — a Tier-A self-read by default,
a Tier-B single-pass dev-spec-reviewer audit on demand. Use zb-spec upstream when the idea itself is
...
6. **DONE-flip discipline (for later, not this command):** flipping a task to DONE (5) requires
   verifying every AC first and PATCHing the AC array — see `/zb-task-done`.

## Usage

```
# Simplest — natural-language request, AC drafted by the skill + Tier-A self-review
/zb-task-create add a rate-limit guard to the email endpoints, with tests

# With explicit urgency signal (skill will set priority=HIGH)
/zb-task-create fix the broken GET /api/tasks pagination — urgent, blocking QA

# High-stakes / new-surface task — opt into the deep dev-spec-reviewer pass before POST
/zb-task-create design the pluggable secrets backend (Infisical + .env fallback) review=deep

# Common operator mistake to avoid: "add to milestone X" means open a task ONLY — do NOT implement
/zb-task-create open a task for the milestone-46 skill-eval pass; assign to ms46
```

The Lead can also invoke this via the Skill tool (skill name = `zb-task-create`).

## Namespace mechanism (how this command is named)

This is a Claude Code **skill**: a `SKILL.md` under `.claude/skills/zb-task-create/`. The directory
name (`zb-task-create`) becomes the invoked name (`/zb-task-create`). The `zb-` family is grouped by
the shared prefix (flat skills, no colon sub-namespace). New/edited skill files are picked up after a
Claude Code **restart**.

## Related skills
- `zb-spec` — adversarial PRE-idea hardening of a fuzzy idea (upstream of this; see the boundary note in Step 2.5)
- `zb-task-done` — close a task you just created, once its AC are verified
- `zb-task-update` — mutate fields on an already-created task (status, priority, etc.)
- `zb-milestone-create` — create a milestone to group the new task under
