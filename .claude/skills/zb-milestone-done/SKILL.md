---
name: zb-milestone-done
description: >-
  Close a milestone (set milestone_status='released') the disciplined way — first check that its
  child tasks are actually finished; warn/halt if not. The milestone analogue of /zb-task-done.
...
# /zb-milestone-done — release a milestone after verifying its tasks
...
2. Don't release a milestone with unfinished child tasks without explicit operator confirmation
   (Step 3 gate — mirrors /zb-task-done's AC gate).

## Related skills
- `zb-milestone-create` — create a new milestone to replace or follow the one you're releasing
- `zb-milestones` — review all milestone progress percentages before deciding to release
