...
- **tn-jobs** — owns job-search LOGIC (classify, dedup, tracker writes, reconcile).
  tn-email owns email MECHANICS only. When `sweep-jobs` needs reconcile/dedup logic,
  hand off to `tn-jobs reconcile`. Do NOT duplicate tracker write logic here.
- **tn-report** — Kanban activity-rail posts (secretary project id=599 or active project).
  tn-email does NOT post to the activity rail; Lead does after email actions complete.
