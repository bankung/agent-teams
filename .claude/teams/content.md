...
This playbook orchestrates the Content team. For universal Lead rules, see root `CLAUDE.md`. This file covers content-specific roster, lifecycle, and conventions.
...
Definitions: [.claude/agents/](.claude/agents/) (the `content-*` + `thai-proofreader` files).
...
4. **Spawn via the Agent tool** — see [.claude/docs/spawn-template.md](.claude/docs/spawn-template.md). Independent passes (e.g. hook-doctor + seo-optimizer on a locked body) can spawn in parallel; the writer→editor revision loop is sequential.
