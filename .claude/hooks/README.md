...
> **One dispatcher, both shells (#3327).** `settings.json` wires a SINGLE PreToolUse entry on matcher `Bash|PowerShell`: `pretooluse-bash-gate.ps1` (name historical — it serves both). It carries in-process MIRRORS of the four `block-*.ps1` guards below as GUARD 2/3/4/5, then runs `approval-policies-gate` (GUARD 6) **for Bash only** — that gate fails open to *ask* on an unbound session, which on the PowerShell path would prompt on every ordinary call.
>
...
| `block-curl-delete.ps1` | ASK on an HTTP DELETE so subagents can't delete API resources outside the Lead-approved flow. Covers curl `-X DELETE` / `--request DELETE`, and (#3327) the PowerShell cmdlets `Invoke-RestMethod` / `Invoke-WebRequest` / `irm` / `iwr` with `-Method DELETE` in any argument order, value quoted or bare. The cmdlets are matched by word boundary, not first word, because the native shape assigns the result (`$r = Invoke-WebRequest ...`). | (see hook source) |
| `block-bitdefender-triggers.ps1` | DENY command shapes Bitdefender heuristics treat as malware (false-positive triggers that nuke the executable). | (see hook source) |
| `block-pytest-on-live-db.ps1` | DENY `pytest` invocations targeting the live `agent_teams` DB. Covers four paths: (1) parent-shell `DATABASE_URL` env not ending in `_test`; (2) an inline `DATABASE_URL` set inside the command string and not ending in `_test` — both the bash prefix form `DATABASE_URL=<url> cmd` and (#3327) the PowerShell form `$env:DATABASE_URL = "<url>"; cmd`, which has spaces around `=` and a quoted value (L1.5); (3) `docker compose exec ... pytest` unless `DOCKER_PYTEST_VERIFIED=1` is set after the operator has manually confirmed the container env (L1.5); (4) `python -c "...pytest..."` denied outright as a likely bypass attempt (L1.5). `python -m pytest` is treated the same as bare `pytest`. Unset `DATABASE_URL` + no bypass triggers is ALLOWED (conftest's in-process rewrite handles it). Codified after the 2026-05-17 dev DB wipe — see `context/projects/agent-teams/shared/incidents/2026-05-17-dev-db-wipe.md`. L1.5 extension: Kanban #1119. | Set `BYPASS_LIVE_DB_PYTEST_HOOK=1` in the SAME shell. The hook honours the bypass and writes a `[BYPASS] ...` marker to stderr for the audit trail. For `docker compose exec ... pytest` specifically, the narrower attestation valve is `DOCKER_PYTEST_VERIFIED=1` — set ONLY after running `docker compose exec api printenv DATABASE_URL` and confirming a `_test` suffix. |
