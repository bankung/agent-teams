    ...
    approval_policies = @{
        rules = @(
            ...
            @{
                ...
                match = @{ tool_name = 'mcp__claude-in-chrome__click' }
...
$tests = @(
    ...
    @{
        Name = 'C3 requires_attention rule on mcp__claude-in-chrome__click'
        Input = '{"tool_name":"mcp__claude-in-chrome__click","tool_input":{"url":"https://example.com","selector":"#submit"}}'
        Policy = $rulesPolicyFile
        ProjectId = $projectIdFile
        Expected = 'ask'
    },
