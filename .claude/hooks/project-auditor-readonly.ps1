...
#
# Per-agent scope:
#   This hook only fires when .claude/agents/project-auditor.md wires it via
#   frontmatter `hooks: PreToolUse:` (per the per-agent-hooks pattern). It does
#   NOT live in .claude/settings.json — other agents (Lead, dev-*, etc.) are
...
#   - .claude/hooks/block-spawn-on-killed-project.ps1 (PS 5.1 + fail-open style)
#
...
if ($writeTools -contains $toolName) {
    Deny-Tool @"
project-auditor is read-only. Tool '$toolName' is denied by .claude/hooks/project-auditor-readonly.ps1.
...
See: .claude/hooks/project-auditor-readonly.ps1
"@
        }
    }
