...
## ⚠️ CRITICAL: Restart Claude Code After Saving

**New agent files are NOT loaded until you restart the Claude Code session.** After you save your agent file in step 4, close and reopen Claude Code or refresh the browser window. Then test immediately (step 5).

## Why Add a Custom Agent?

Claude Code supports "agents" — roles with specialized instructions, allowed tools, and fixed model choice. A custom agent lets you:
...
Agent files live in `.claude/agents/` directory, one file per agent.
...
If the `.claude` folder doesn't exist yet, create it at your project root, then create `agents` inside it.
...
1. **Close and reopen Claude Code** (or reload the browser window).
...
- Claude Code reads agent files when the session starts, not continuously

**Fix:**
1. Close the Claude Code window completely
2. Reopen Claude Code (or refresh the browser tab)
...
- Claude Code uses the name to identify which agent to run
...
2. Look at an existing agent in `.claude/agents/` to compare structure
3. Ask Claude Code directly — it can often spot YAML errors or naming issues

---

<!-- SCREENSHOT PLACEHOLDERS FOR OPERATOR:
1. Screenshot of .claude/agents/ folder in file explorer (showing a few existing .md files)
...
Destination: .claude/docs/img/ — operator adds these and updates image refs in this markdown file.
-->
