...
- **Meta-orchestrator + team playbooks.** A "Lead" agent resolves the active project, loads that project's team playbook, and spawns the right domain specialists for the work. **8 teams** (dev, novel, general, content, netops, SEO, data-analytics, SEM) and **~39 specialist agent definitions** live in `.claude/agents/`, each with a scoped lane and permission model.
