    ...
    Tiers:
      ...
      l2   Pro pilot preset. Downgrade routine agents to conserve Opus quota:
           ...
           Stays Opus regardless of tier:
             dev-sr-backend, dev-sr-frontend  (sr-* roles)
             bi-analyst, sem-campaign-lead, seo-strategist  (strategists)
...
switch ($Tier) {
    ...
    { $_ -in @('', '-h', '--help') } {
        Write-Host @"
Usage: .\bin\agent-teams-tier-set.ps1 max|l2
...
"@
    ...
    default {
        Write-Host "ERROR: Unknown tier: $Tier. Use max|l2." -ForegroundColor Red
        exit 1
    }
}
