...
Use the `Agent` tool with `subagent_type` matching the role. Available role names depend on the active team's roster — see `.codex/teams/<active.team>.md`. Examples:
...
- If the task has `acceptance_criteria` (check `curl -H "X-Project-Id: <id>" /api/tasks/<id>`), your final report MUST include a per-criterion verdict table + JSON block matching the shape in `.codex/agents/dev-tester.md` `### 2d` or `.codex/agents/dev-reviewer.md` "Acceptance criteria audit" bullet. NEVER report status='pending' — that means you didn't check.
...
- `[content-team] Draft .codex/teams/content.md playbook`
