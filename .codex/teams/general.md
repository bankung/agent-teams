...
This playbook orchestrates the general team. For universal Lead rules, see root `AGENTS.md`. This file covers general-specific roster, decision logic, and lifecycle.
...
Definitions: [.codex/agents/general.md](.codex/agents/general.md) (the fallback); [.codex/agents/](.codex/agents/) (all available specialists).

### When to spawn the `general` agent

1. **Ambiguous scope** — task description doesn't clearly name a domain (e.g., "figure out why the deploy is slow" is investigation).
2. **Cross-stack / multi-domain** — a single specialist wouldn't own the full scope end-to-end.
3. **One-off scripting** — e.g., "write a data migration helper", "extract email analytics from logs".
4. **Fallback decision tree** — you've considered the specialists and none fit well.

**Anti-pattern: spawning `general` to avoid domain decision.** If the task description says "add a login feature", that's dev-backend + dev-frontend, not `general`.

### When to spawn a specialist instead

- **Task names the domain clearly** — "fix the POST /tasks endpoint" → dev-backend. "rewrite the dashboard cards" → dev-frontend. "update the CI config" → dev-devops.
- **Specialist owns the outcome** — if the task's success is measured by code quality in a domain, that specialist's best tooling + discipline applies.
...
**When spawning a specialist from within a general project:** the specialist inherits the same lane mapping as they would in a dev-team project (e.g., dev-frontend gets `standards.web`). Lead passes the lanes in the spawn brief.

## Kanban schema codes (`tasks.assigned_role`)

The general team **inherits the dev team's codes** for any dev-* specialist roles. When a task is assigned to `general` itself:
...
The design: `assigned_role=NULL` signals that this task started with the `general` agent. If `general` escalates (e.g., discovers the work is heavily React-focused), Lead spawns dev-frontend and re-patches the task with `assigned_role=1`.
...
2. **Read relevant context**:
   ...
   - `general/current-state.md` if present
   - `standards/general.md` always; any framework standards per the task's domain hints
3. **Assess task scope — decide on role(s)**:
   - **If clearly domain-specific** (the task description names a specialist's domain): spawn that specialist directly.
   - **If ambiguous or cross-stack**: spawn `general`. The agent may escalate mid-task if the work is squarely in a specialist's domain.
3b. **Research-first discipline — standing rule.** Before spawning the chosen role(s), check whether the task crosses a "non-trivial" threshold. When yes → spawn `general-researcher` (Haiku tier) FIRST or in the first parallel batch; its summary feeds the specialist brief.

   **General-team "non-trivial" signals:**
   - Cross-stack scope (one fix touches schema + API + FE + tests + standards).
   - Unfamiliar libraries / APIs / external services.
   ...
   **Escape valves (skip research):**
   - Pure execution (typo, one-off script with crystal-clear scope).
   - Continuation of an already-researched task.
   - Trivial single-edit follow-up to a still-open task.

4. **Spawn via the Agent tool** — see [.codex/docs/spawn-template.md](.codex/docs/spawn-template.md). Include the escalation protocol: `general` should STOP and escalate if they discover domain boundaries.
5. **Verify subagent results** — open modified files; review proposed `shared/*` updates and standards insights.
6. **Smoke testing (if applicable)** — when the task touched API / schema / Docker / CI: spawn dev-tester to run scoped probes. Methodology: [`context/teams/dev/smoke-methodology.md`](../../context/teams/dev/smoke-methodology.md) and the project's `shared/smoke-matrix.md`.
7. **Escalation handling** — if `general` escalates in their final report:
   - **Create a handoff note:** `context/projects/<active>/general/escalation-<task-slug>.md`.
   - **Spawn the specialist** with the escalation note as context.
   - **Update the Kanban task** with the specialist's assigned_role code.
...
9. **Update task status in the DB**: `PATCH /api/tasks/<id>` with `process_status=2` + `started_at` on start; `process_status=5` + `completed_at` on done.
10. **Handoff or close** — if an escalation occurred, the specialist's return signals the task is done. Otherwise summarize to the user (2–3 sentences).
11. **Compaction** — the `general` agent updates `context/projects/<active>/general/current-state.md` before returning.

## General-specific anti-patterns

- **Spawning `general` when the task description names a specialist** — "add a POST endpoint" is dev-backend, not `general`.
- **`general` powering through a domain task** — if mid-task they realize the work is React-heavy, escalate immediately.
...
- [.codex/agents/general.md](.codex/agents/general.md) — the fallback agent's full scope and escalation protocol.
- [.codex/teams/dev.md](.codex/teams/dev.md) — specialist roster when spawning dev-* agents.
- [.codex/teams/novel.md](.codex/teams/novel.md) — specialist roster when spawning novel-* agents.
