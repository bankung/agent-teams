...
This playbook orchestrates the dev team. For universal Lead rules, see root `AGENTS.md`. This file covers dev-specific roster, lifecycle, and conventions.
...
Definitions: [.codex/agents/](.codex/agents/) (the `dev-*` and `dev-sr-*` files).
...
4. **Spawn via the Agent tool** — see [.codex/docs/spawn-template.md](.codex/docs/spawn-template.md). Independent roles can spawn in parallel.
