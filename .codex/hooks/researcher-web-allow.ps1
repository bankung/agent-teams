...
#
# Scoped via .codex/agents/general-researcher.md frontmatter (PreToolUse on WebFetch + WebSearch).
# Other roles (Lead, dev-backend, dev-frontend, dev-devops, dev-tester, dev-reviewer,
# dev-documentor) do NOT inherit this hook — they fall through to hooks.json's
...
#   anything else              -> neutral (exit 0, no JSON; hooks.json normal flow)
#
...
if ($toolName -eq 'WebSearch') {
    $output = @{
        hookSpecificOutput = @{
            ...
            permissionDecisionReason = "WebSearch auto-approved for general-researcher (.codex/hooks/researcher-web-allow.ps1)"
        }
    ...
    if ($url -match '^https://') {
        $output = @{
            hookSpecificOutput = @{
                hookEventName            = "PreToolUse"
                permissionDecision       = "allow"
                permissionDecisionReason = "https WebFetch auto-approved for general-researcher (.codex/hooks/researcher-web-allow.ps1)"
            }
        } | ConvertTo-Json -Compress -Depth 4
        Write-Output $output
        exit 0
