...
#       .codex/agents/<agent_type>.toml declares email_actions = "enabled"
#       (or legacy .md frontmatter declares email_actions: enabled)
#                                                      -> allow
...
# call originates from a subagent (Codex subagent hook contract). Absent =
# main session.
...
# Named subagent. Authorize ONLY if it is a secretary* agent whose agent config
...
$agentTomlFile = Join-Path $agentsDir ("{0}.toml" -f $agentType)
$agentMarkdownFile = Join-Path $agentsDir ("{0}.md" -f $agentType)

$isSecretaryName = ($agentType -match '^secretary')
$hasSignal = $false
if (Test-Path $agentTomlFile) {
    try {
        $agentText = Get-Content -Raw -Path $agentTomlFile
        # TOML signal: a top-level `email_actions = "enabled"` key in the
        # converted Codex agent config.
        if ($agentText -match '(?im)^\s*email_actions\s*=\s*["'']?enabled["'']?\s*$') { $hasSignal = $true }
    } catch {
        Fail-Open-Ask -WarnMsg "cannot read agent file ${agentTomlFile}: $($_.Exception.Message)"
    }
} elseif (Test-Path $agentMarkdownFile) {
    try {
        $agentText = Get-Content -Raw -Path $agentMarkdownFile
        ...
        Fail-Open-Ask -WarnMsg "cannot read agent file ${agentMarkdownFile}: $($_.Exception.Message)"
    }
}
...
$why = if (-not $isSecretaryName) { "non-secretary agent '$agentType'" } else { "agent '$agentType' lacks email_actions:enabled config marker" }
