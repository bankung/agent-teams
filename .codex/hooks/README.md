...
| --- | --- | --- |
| `block-raw-sql-dml.ps1` | DENY `psql -c` / `python -c` calls that contain DELETE/UPDATE/INSERT/TRUNCATE/DROP/ALTER TABLE. Diagnostic SELECT/`\d`/EXPLAIN pass through. Codified rule: see `.codex/docs/lessons.md` "Raw SQL DML is human-only" (strike #1 Kanban #483, 2026-05-09). | Edit hooks.json to remove the hook entry, or run the command in a terminal outside Codex. |
...
## Per-agent permission hooks (loaded via subagent frontmatter, not hooks.json)
