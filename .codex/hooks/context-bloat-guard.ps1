...
#
# Wired as a PostToolUse(Write|Edit) hook (.codex/hooks.json). Loads at
# session start.

$root = if ($env:CODEX_PROJECT_DIR) { $env:CODEX_PROJECT_DIR } else { (git rev-parse --show-toplevel 2>$null) }
if (-not $root) { exit 0 }
