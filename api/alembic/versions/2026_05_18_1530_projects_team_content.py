"""projects.team add 'content' (Kanban #971)

Revision ID: 0038_projects_team_content
Revises: 0037_tasks_requires_human_review
Create Date: 2026-05-18 15:30 UTC

Adds `'content'` to the `ck_projects_team_valid` CHECK constraint so projects
can opt into the content-team playbook (article / social / newsletter / general
writing work). The team enum is a wire-contract that must land atomically
across BE (constants.py, ORM CheckConstraint, scaffolds, the TeamCode Literal
guard in schemas/project.py) and FE (web/lib/constants.ts) — splitting risks a
window where one side knows about the new value but the other rejects it.
...
recreate is the canonical idiom and is what `0021_projects_team_general` used
when it last touched this constraint.

Downgrade caveat: the recreated CHECK in `downgrade()` only allows
`('dev','novel','general')`. If any rows with `team='content'` exist at
...
revision = "0038_projects_team_content"
down_revision = "0037_tasks_requires_human_review"
branch_labels = None
depends_on = None
...
# app and migration".
_PROJECT_TEAM_ALL_NEW = ("dev", "novel", "general", "content")
_PROJECT_TEAM_ALL_OLD = ("dev", "novel", "general")
...
def downgrade() -> None:
    # CAVEAT: any row with team='content' will block the recreate. Operator
