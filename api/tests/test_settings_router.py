"""Integrations settings router contract smoke tests.

First-pass contract coverage for GET /api/settings/integrations (read-only;
no toggle endpoint exists). The rigorous suite — per-category matrices, every
integration's required-var permutations — is dev-tester's domain.

Contract shape: each integration has {id, label, category, configured, env_vars,
setup}. There is NO `enabled` field. There is NO PATCH endpoint.

  (a) `configured` is False when a required env var is absent, True when all
      required vars are monkeypatched present.
  (b) GET never leaks a secret VALUE — only presence booleans.
  (c) platform_security.vault_key_configured reflects CREDENTIALS_MASTER_KEY.

The router is GLOBAL (no X-Project-Id header). It makes NO DB queries — only
os.environ reads — so the conftest live-DB invariant remains green.
"""
...
# A recognisable secret value used by the no-leak assertion (b). If the response
...
# Required env vars per integration (mirrors the registry — kept here so the
# test is self-documenting and fails loudly if the registry's required set drifts).
...
async def test_configured_false_when_required_env_absent(client, monkeypatch):
    ...
    assert "enabled" not in anthropic, "enabled field must not exist in response"
...
# ---------------------------------------------------------------------------
# (c) platform_security.vault_key_configured — Kanban #1658.
# ---------------------------------------------------------------------------
