...
# ---------------------------------------------------------------------------
# Kanban #1300 (smoke artifact, parent #1293) — ProjectCreate.name + ProjectUpdate.name
# explicit max_length validation. Length-first 422 message discipline.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_project_name_64_chars_accepted(client, scaffold_cleanup) -> None:
    """AC (a): a 64-char name (the boundary) is accepted with 201.

    Smoke-locks the upper boundary of the explicit max_length=64 constraint on
    ProjectCreate.name. The name uses only `a` chars to stay within the regex
    character class; the unique suffix keeps it below 64 chars.
    """
    # 56 'a' chars + 8-hex suffix = 64 chars total.
    suffix = uuid.uuid4().hex[:8]
    name = "a" * 56 + suffix
    assert len(name) == 64, f"test setup: expected 64 chars, got {len(name)}"
    scaffold_cleanup(name)
    payload = _project_create_payload(name)
    resp = await client.post("/api/projects", json=payload)
    assert resp.status_code == 201, (
        f"expected 201 for 64-char name, got {resp.status_code}: {resp.text}"
    )
    assert resp.json()["name"] == name


@pytest.mark.asyncio
async def test_project_name_65_chars_rejected_422(client) -> None:
    """AC (b): a 65-char name (one over the boundary) is rejected with 422.

    Smoke-locks that the explicit max_length=64 constraint fires and produces
    a 422 (not a 200/201 or a 500). No DB row is created so no cleanup needed.
    """
    suffix = uuid.uuid4().hex[:8]
    name = "a" * 57 + suffix  # 57 + 8 = 65 chars
    assert len(name) == 65, f"test setup: expected 65 chars, got {len(name)}"
    payload = _project_create_payload(name)
    resp = await client.post("/api/projects", json=payload)
    assert resp.status_code == 422, (
        f"expected 422 for 65-char name, got {resp.status_code}: {resp.text}"
    )


@pytest.mark.asyncio
async def test_project_name_65_chars_error_mentions_length(client) -> None:
    """AC (c): the 422 error body for a 65-char name mentions 'length' / 'characters'.

    Smoke-locks that the error message from the explicit max_length=64
    constraint is legible — it must name the length constraint (Pydantic v2
    emits 'String should have at most 64 characters'), NOT the opaque
    'String should match pattern' that the regex alone would produce.
    """
    name = "a" * 65
    assert len(name) == 65
    payload = _project_create_payload(name)
    ...
    assert resp.status_code == 422, resp.text
    body = resp.json()
    # Extract all error messages from the Pydantic 422 body.
    errors = body.get("detail", [])
    assert isinstance(errors, list) and errors, (
        f"expected list of errors in detail, got {body!r}"
    )
    msgs = " ".join(e.get("msg", "") for e in errors).lower()
