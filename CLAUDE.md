...
4. **Persist (session-scoped, #2679):** get `sid` via `echo $CLAUDE_CODE_SESSION_ID` (= the session_id the cost-capture hooks see), then write `<id>` (single integer) to BOTH `_runtime/lead_project_id_<sid>.txt` (the per-session binding the cost-capture hooks read — UUID-keyed so a stale file can't be mis-read) AND `_runtime/lead_project_id.txt` (legacy global; since #2692/#2680 read ONLY by the SEO audit hook ([seo-ranking-report.ps1](.claude/hooks/seo-ranking-report.ps1)/.sh, session-less) — the gate/notify/spawn hooks resolve per-session via `Get-ProjectId -SessionId`, and tn-* skills via `bin/lead-project-id.ps1`; KNOWN-GAP-1 #2694).
...
6. **Explicit mid-session switch:** RE-bootstrap from step 2 — discard prior context, re-read the new project's `shared/*` + team playbook (if team differs), re-write the binding (both `_runtime/lead_project_id_<sid>.txt` and the legacy global `_runtime/lead_project_id.txt`).
