...
5. **Token economy.** Scoped checks + ONE full test run by default; reserve repeated/≥15× determinism loops for tasks whose AC demands it. Don't re-read a file Edit/Write already confirmed; keep spawn briefs lean; don't spawn a cold agent for what Lead can already do correctly (see golden-rule carve-out E).
   - **A — batch the FE loop.** Where web has no bind mount, source changes need a full image rebuild — take the compose project name from the bound project's `shared/README.md` ("Project facts (compose + tests)"), never a hard-coded one. Batch all visual edits, then rebuild + verify ONCE per batch — never per individual change.
...
- **Pytest briefing discipline.** Any spawn mentioning `pytest` MUST include an explicit AC: *"report the live DB row count BEFORE and AFTER the pytest invocation."* Which DB is live and which is the test DB comes from the bound project's `shared/README.md` ("Project facts (compose + tests)"). Lead independently verifies via `curl` BEFORE flipping task status to DONE. See [lessons.md](.claude/docs/lessons.md) "Pytest briefing discipline."
...
**Two records, two questions.** The **activity rail** answers *"what happened in this task"* — immutable, terse, one `/zb-report` POST per checkpoint. A **story doc** (`<shared>/stories/<slug>.md`, Lead-only writer) answers *"what is true NOW for this thread"* — updated in place at every related task close. Never duplicate a fact between them.
...
- **Context hygiene (warm vs clear):** stay warm within a batch/chain (same files/module · consumes prior output/contract · FK link · sequential milestone slices) — but re-verify obsolescence + code state at EVERY task start regardless. Recommend the operator clear / new-session at: new surface · release/milestone close · noisy debug stretch · ≥2 in-session compactions with no chain ahead · project switch · a long/high-turn session once its thread is done (accumulated per-turn context = the dominant token cost — tail ≈ 2–3× average; shared/design/leanness-review-plan.md #2728). State the recommendation (+ in-session compaction count) in the end-of-engagement summary.

Mechanics — story-doc template + versioning/optimistic-lock, pickup-read 4-layer fallback, write-side `from #X` duty, sunset eval RESOLVED-FORWARD via #2520 (sharpen-not-remove; residual = light stick-check at the next 2-3 story-eligible threads): [.claude/docs/context-lifecycle.md](.claude/docs/context-lifecycle.md).
...
2. **Resolve via API:** `curl --silent http://localhost:8456/api/projects/by-name/<name>`. 404 → list live projects, ask again. API down → re-seed the harness stack with `docker compose -p <compose name> exec -T api python -m scripts.seed`, compose name from the agent-teams project's `shared/README.md` — never a bare `docker compose exec`, which hits whichever stack the current folder belongs to. Retry; still failing → check Docker + FastAPI logs, stop and wait.
...
4. **Persist (session-scoped, #2679):** run `/zb-bind` — it resolves the project via the API and writes both the per-session binding (`_runtime/lead_project_id_<sid>.txt`, UUID-keyed so a stale file can't be mis-read) and overwrites the GLOBAL one-way signal (`_runtime/lead_project_id.txt`) the session-less Telegram HITL poller daemon [telegram_poller.py](api/scripts/telegram_poller.py) follows (env `TELEGRAM_POLLER_PROJECT_ID` first, this file as fallback; #2565). Sessions WRITE/overwrite the global, never READ it; every interactive reader (gate/notify/spawn hooks + zb-* skills) resolves per-session via `Get-ProjectId -SessionId` / `bin/lead-project-id.ps1` — mechanics live in the `zb-bind` skill, not restated here.
5. **Read the team playbook** `.claude/teams/<team>.md` — authoritative for roster, lane mapping, lifecycle, anti-patterns — and the bound project's `<working_path>/shared/README.md`, whose "Project facts (compose + tests)" section is the ONLY source for that project's compose project name, test command and test database (missing → create it before any build or test command). Read only the hot set at bootstrap (decisions-index + api-contracts-core + state digest); pull big refs on demand (lazy-read doctrine #1798).
6. **Explicit mid-session switch:** RE-bootstrap from step 2 — discard prior context, re-read the new project's `shared/*` + team playbook (if team differs), re-write the binding (the per-session `_runtime/lead_project_id_<sid>.txt`, and overwrite the global `_runtime/lead_project_id.txt` — never read it).
...
Adding a team is agent-teams platform work — it edits that repo's own source. The step-by-step checklist lives in the agent-teams project's shared docs, in `shared/adding-a-team.md`.

**Research-first:** non-trivial tasks open with a researcher spawn (Haiku) before the specialist — per-team "non-trivial" heuristics + escape valves live in each playbook.
