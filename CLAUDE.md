...
4. **Persist (session-scoped, #2679):** get `sid` via `echo $CLAUDE_CODE_SESSION_ID` (= the session_id the cost-capture hooks see), then write `<id>` (single integer) to BOTH `_runtime/lead_project_id_<sid>.txt` (the per-session binding the cost-capture hooks read — UUID-keyed so a stale file can't be mis-read) AND `_runtime/lead_project_id.txt` (the GLOBAL one-way signal the session-less Telegram HITL poller daemon [telegram_poller.py](api/scripts/telegram_poller.py) reads to follow the active project — env `TELEGRAM_POLLER_PROJECT_ID` first, this file as fallback; #2565). Sessions WRITE/overwrite the global, never READ it; every interactive reader (gate/notify/spawn hooks + zb-* skills) resolves per-session via `Get-ProjectId -SessionId` / `bin/lead-project-id.ps1`.
