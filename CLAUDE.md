...
4b. **Persist the binding for the hook layer (Kanban #1209):** write the bound `<id>` (single integer, no surrounding whitespace required) to `_runtime/lead_project_id.txt`, overwriting any prior content. This file is the contract read by `.claude/hooks/block-spawn-on-killed-project.ps1` — every Agent spawn checks the bound project's `is_killed` state via `GET /api/projects/<id>` and denies the spawn if the project is killed. The hook fails open (with stderr WARN) if the file is missing, so Lead-direct work continues, but the spawn-block layer is INACTIVE until this step runs. Use the Bash tool: `echo <id> > _runtime/lead_project_id.txt` (cwd = repo root). Lazy-discoverable for future Lead sessions via `_runtime/.gitkeep` + `.gitignore` (`_runtime/*` ignored, `.gitkeep` kept).
...
6. **Explicit mid-session switch** ("actually let's switch to myapp"): Lead RE-bootstraps from step 2 with the new name — discards in-memory context of the prior project, re-reads the new project's `shared/*`, re-loads the team playbook if the team differs. **Step 4b applies:** re-write `_runtime/lead_project_id.txt` with the new id so the spawn-block hook scopes to the new binding.
...
- Lifecycle-program lock-code keywords in committed files → see `_scratch/.lifecycle-mapping.md` for substitutions. The pre-push hook (`.git/hooks/pre-push`) enforces forward-prevention; existing history may carry pre-policy leaks (out-of-scope to scrub).
