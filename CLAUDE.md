...
4. **Review-to-rewrite before finalizing.** On a non-trivial diff, run `/simplify` (reuse / simplification / efficiency / altitude pass that *applies* fixes — our review-rewrite) and `/code-review` for bug-hunting; prefer these over re-deriving by hand.
5. **Token economy.** Default to scoped checks + ONE full test run; reserve repeated/≥15× determinism loops for tasks whose AC explicitly demands it. Batch rebuilds. Don't re-read a file Edit/Write already confirmed. Keep spawn briefs lean.
