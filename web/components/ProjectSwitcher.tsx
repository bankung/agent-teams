  ...
  return (
    <div ref={wrapperRef} className="relative inline-block" data-project-switcher>
      ...
      {open && (
        <div
          role="listbox"
          className="absolute left-0 top-full z-20 mt-1 w-64 rounded border border-zinc-200 bg-white py-1"
          data-project-switcher-panel
        >
          {loadError !== null && (
            <div className="px-3 py-2 text-xs text-red-700">{loadError}</div>
          )}
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between gap-2 px-3 py-1.5 text-left text-sm hover:bg-zinc-50 ${
                  isCurrent ? "text-zinc-900 font-medium" : "text-zinc-600"
                }`}
                data-project-name={p.name}
              >
                <span className="truncate">{p.name}</span>
                <span className="inline-flex shrink-0 items-center rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-zinc-600">
                  {p.team}
                </span>
              </button>
            );
