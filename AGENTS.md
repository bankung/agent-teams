...
Drift catalog + 4 modes + incident history: see [.codex/docs/lessons.md](.codex/docs/lessons.md) "Karpathy lane (universal discipline on every turn)".
...
- **DB writes go through FastAPI endpoints only.** No `psql` or ad-hoc ORM scripts. See [.codex/docs/lessons.md](.codex/docs/lessons.md) "Raw SQL DML is human-only."
- **Pytest briefing discipline.** Any spawn mentioning `pytest` MUST include an explicit AC: *"report live `agent_teams` DB row count BEFORE and AFTER the pytest invocation."* Lead independently verifies via `curl` BEFORE flipping task status to DONE. See [.codex/docs/lessons.md](.codex/docs/lessons.md) "Pytest briefing discipline."
...
Auto-memory (`~/.codex/projects/<cwd-hash>/memory/`) lives OUTSIDE the 5 zones above — it's operator-personal, scoped to Codex's CWD not Lead's project binding. A session in agent-teams writes here even when bound (via API) to project=secretary.
...
The Codex harness plus `.codex/hooks.json` enforce:
...
Each Codex session is **bound to one project** for its entire lifetime.
...
4. **Announce binding:** "Session bound to <name> (team=<team>, id=<id>)."
   ...
   From this point, every API call to `/api/tasks*` MUST include `-H "X-Project-Id: <id>"`. Subagent spawn briefs must mention the convention (see [.codex/docs/spawn-template.md](.codex/docs/spawn-template.md)).

5. **Persist the binding:** write `<id>` to `_runtime/lead_project_id.txt` (single integer). This file is read by the spawn-block hook — see [.codex/hooks/block-spawn-on-killed-project.ps1](.codex/hooks/block-spawn-on-killed-project.ps1) for details.
6. **Read the team playbook:** `.codex/teams/<team>.md`. Treat as authoritative for roster, lane mapping, lifecycle, and domain anti-patterns.
...
Dev team tracks subagent tier in `tasks.subagent_models` per Kanban #887 — full spec in [.codex/teams/dev.md](.codex/teams/dev.md).
...
All team playbooks below are domain extensions of AGENTS.md's universal rules — **read AGENTS.md first.** Each team file covers domain-specific roster, lifecycle, lane mapping, and conventions.

- [`.codex/teams/dev.md`](.codex/teams/dev.md) — software development (the agent-teams repo itself uses this).
- [`.codex/teams/novel.md`](.codex/teams/novel.md) — novel writing (skeleton; demonstrates multi-domain pattern).
- [`.codex/teams/general.md`](.codex/teams/general.md) — general-purpose / fallback; for projects that don't fit a single domain.
- [`.codex/teams/seo.md`](.codex/teams/seo.md) — SEO strategy, technical audit, content optimization, reporting.
- [`.codex/teams/sem.md`](.codex/teams/sem.md) — paid media (Google Ads, Meta, secondary platforms, campaign strategy).
- [`.codex/teams/data-analytics.md`](.codex/teams/data-analytics.md) — BI analysis, SQL, dashboard design, integration.
